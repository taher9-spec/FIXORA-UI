...
export async function POST(request: Request) {
  try {
    const { redirectUri, scopes = ["profile", "email"] } = await request.json()
    ...
    // In a real implementation, you would register a Google OAuth app
    const clientId = "mock-google-client-id"

    // For demo purposes, we'll simulate the OAuth flow entirely
    // Instead of redirecting to Google, we'll redirect directly to our callback
    ...
    console.error("Google OAuth credentials error:", error)
